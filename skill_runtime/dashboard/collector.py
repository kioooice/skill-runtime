...
from skill_runtime.operator_visibility import (
    ...
    OPERATOR_SUMMARY_STALE_AFTER_SECONDS,
    coerce_freshness_policy,
    enrich_operator_summary_freshness,
...
def collect_dashboard_operator_summary_data(root: str | Path) -> dict[str, Any]:
    ...
    return {
        ...
        "freshness_policy": freshness_policy_export(OPERATOR_SUMMARY_STALE_AFTER_SECONDS),
...
def _load_exported_operator_summary(root: Path, diagnostics: list[str] | None) -> dict[str, Any] | None:
    ...
    return enrich_operator_summary_freshness(payload) if isinstance(payload, dict) else None
