...
class RuntimeSearchQualityTestsMixin:
    def test_search_quality_evaluation_script_does_not_write_operator_status_by_default(self) -> None:
        with tempfile.TemporaryDirectory(prefix="search-quality-operator-status-default-") as temp_dir:
            ...
            self.assertFalse(
                (status_root / ".skill_runtime" / "operator_status" / "search_quality.json").exists()
            )

    def test_search_quality_evaluation_script_can_write_operator_status(self) -> None:
        with tempfile.TemporaryDirectory(prefix="search-quality-operator-status-") as temp_dir:
            status_root = Path(temp_dir)
            result = subprocess.run(
                [
                    sys.executable,
            ...
            self.assertEqual(0, result.returncode, msg=result.stderr or result.stdout)
            status_file = status_root / ".skill_runtime" / "operator_status" / "search_quality.json"
            ...
            self.assertEqual(7, payload["summary"]["query_count"])
            self.assertEqual(7, payload["baseline_comparison"]["matched"])
