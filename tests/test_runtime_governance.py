...
class RuntimeGovernanceTestsMixin:
    ...
    def test_archive_fixture_skills_preserves_late_same_skill_index_updates(self) -> None:
        ...
        original_archive_skill_metadata = sandbox_service._archive_skill_metadata
        late_updated = {"done": False}

        def archive_with_late_same_skill_update(metadata):
            archived = original_archive_skill_metadata(metadata)
            if archived and not late_updated["done"]:
                stable_metadata = sandbox_index.get("merge_text_files")
                self.assertIsNotNone(stable_metadata)
                stable_metadata.summary = "Late-updated stable skill summary."
                stable_metadata.rule_reason = "late same-skill update"
                sandbox_index.upsert(stable_metadata)
                late_updated["done"] = True
            return archived

        sandbox_service._archive_skill_metadata = archive_with_late_same_skill_update
        self.addCleanup(setattr, sandbox_service, "_archive_skill_metadata", original_archive_skill_metadata)

        sandbox_service.archive_fixture_skills(skill_names=["cli_merge_fixture_test"])

        refreshed_index = sandbox_index.__class__(sandbox_root / "skill_store" / "index.json")
        stable_skill = refreshed_index.get("merge_text_files")
        self.assertIsNotNone(stable_skill)
        self.assertEqual("Late-updated stable skill summary.", stable_skill.summary)
        self.assertEqual("late same-skill update", stable_skill.rule_reason)
