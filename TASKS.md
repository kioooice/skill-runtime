...
- 最新 search quality baseline：已新增 `docs/v0.2-search-quality-plan.md`，并将 `scripts/evaluate_search_quality.py` 收口成临时 runtime root + 小型 active skill fixture 子集 + 固定 query set 的本地评估脚本。当前它不依赖外部服务，不污染真实 `skill_store`，并诚实报告 query 命中情况而不是把“全绿”当唯一结果。当前 baseline 覆盖 exact English、fuzzy English、structured conversion、maintainer workflow、Chinese known-limitation probe 和 negative no-strong-match 六类 query；最新结果为 5/6 matched，唯一失败是纯中文 merge query，说明当前最大问题仍然是 ASCII-style lexical tokenization 对中文召回几乎无覆盖。对应测试已改成契约验证：报告字段完整、至少一个 positive query matched、negative query 不会被记成 fake pass。当前这只是 search/reuse baseline，不是算法重写，也没有任何证据支持扩大 `default-in`。
