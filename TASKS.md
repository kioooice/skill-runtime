...
- 下一步：主线已经纠偏为“开发前方向审核 / 防止无意义开发”。全局 `pre-implementation-workflow-review` 已从 checklist 升级为主流程门禁：只有 `build_now` 允许同一流程进入实现；`manual_validation_first`、`revise_direction` 和 `do_not_build_now` 都必须先验证、改路线或停止。刚才的路线纠错也已再纠偏：具体错误记录不再继续塞进 `AGENTS.md`，而是由全局 `workflow-error-correction` skill 记录错误模式、预防规则和状态文件落点。`AGENTS.md` 只保留短规则和 workflow skill 路由。当前原则已明确：全局 Codex skill 是通用工作流的唯一权威来源，项目只保留路由、局部约束、索引或薄适配。Runtime 相关能力已经够支撑观察和回收经验，继续开发必须直接服务于“审核开发方向是否值得做”。
