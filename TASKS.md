...
- [ ] 在下一次可复用 workflow staging 候选出现时 dogfood `promote-global-codex-skill`，确认全局 skill 写入和新会话触发链路
