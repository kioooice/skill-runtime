...
| Active skill library | Clean but small | Search is no longer polluted by fixtures and demos, and now includes merge, archive, single-file JSON-to-CSV, and directory JSON-to-CSV dogfood samples. | Four real active skills still do not prove broad usefulness. |
