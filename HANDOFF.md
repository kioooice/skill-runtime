...
最新 search quality baseline：已把主线切到本地可复现的 skill search/reuse baseline，并新增 `docs/v0.2-search-quality-plan.md` 与重写后的 `scripts/evaluate_search_quality.py`。当前脚本不再直接跑真实仓库 root，也不再把“全绿”当唯一成功条件；它会在临时 runtime root 中导入一小组 active skill fixtures（当前是 `merge_text_files`、`json_to_csv_dogfood`、`session_handoff_maintenance`、`pre_implementation_workflow_review`、`directory_text_cleanup_dogfood`），再对固定 query set 跑 `RuntimeService.search`，输出结构化 JSON report。当前 report 字段至少包含 `query_id`、`query`、`expected_top_skill` / `expected_skills`、`actual_top_skill`、`actual_recommended_skill`、`matched`、`top_k`、`failure_reason`、`rank_diagnostics` 和 `top_results`。当前 baseline 覆盖 exact English、fuzzy English、structured conversion、maintainer workflow、Chinese known-limitation probe 和 negative no-strong-match 六类 query。最新结果是 `query_count=6`、`matched_count=5`、`positive_matched_count=4/5`、`negative_matched_count=1/1`；唯一未命中是纯中文 query `把多个文本文件合并成一个文档`，当前直接 `No search results returned.`。这暴露的最大问题很明确：`skill_runtime/retrieval/skill_index.py` 目前只做 ASCII-style lexical tokenization，中文查询几乎没有可用召回。当前这只是 search baseline，不是检索算法重写，不是 dashboard 变更，不是 `default-in` 证据，也没有任何证据支持扩大 `default-in`。
