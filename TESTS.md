...
- Full suite result after adding `text_replace_dogfood`: 365 tests passed in about 7 minutes.
...
- Fast suite result after adding `text_replace_dogfood`: 19 tests passed in about 22 seconds.
...
- Search quality baseline: `python scripts/evaluate_search_quality.py`; this checks current active skills against a small expected-query set and is also covered by the fast suite. Current baseline: 11 checks passed.
