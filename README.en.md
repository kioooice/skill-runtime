...
With `dashboard --global`, these refresh flags still apply only to the current `--root`. Scanned sibling projects remain read-only and render from their existing local exports or event files.
